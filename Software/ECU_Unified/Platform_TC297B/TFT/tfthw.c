...
void tft_transmit_callback(void)
{
    ...
    if (g_Qspi_Tft.drivers.spiMaster->dma.useDma == 0)
        if (g_Qspi_Tft.drivers.spiMasterChannel.tx.remaining) return;
...
static void tft_write_data (uint8 regaddr, uint16 data)
{
    ...
    while (IfxQspi_SpiMaster_getStatus(&g_Qspi_Tft.drivers.spiMasterChannel) == IfxQspi_Status_busy) {};
...
void tft_init (void)
{
...

    {
        ...
        spiMasterChannelConfig.ch.baudrate = 50000000;

        /* set the transfer data width */
        spiMasterChannelConfig.ch.mode.dataWidth = 32;
        spiMasterChannelConfig.ch.mode.csLeadDelay = 1;
        spiMasterChannelConfig.ch.mode.csTrailDelay = 0;
        spiMasterChannelConfig.ch.mode.csInactiveDelay = 0;
