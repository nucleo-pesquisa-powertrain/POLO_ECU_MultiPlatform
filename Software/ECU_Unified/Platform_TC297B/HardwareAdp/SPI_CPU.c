...
#define MASTER_CHANNEL_BAUDRATE     2500000         /* Master channel baud rate                                     */
...
void initQSPI2Master(void)
{
    ...
    const IfxQspi_SpiMaster_Pins qspi2MasterPins = {
        ...
        IfxPort_PadDriver_lvdsSpeed4                          /* Pad driver mode                          */
    };
...
void initQSPI2MasterChannel(void)
{
    ...
    const IfxQspi_SpiMaster_Output qspi2SlaveSelect = {                 /* QSPI2 Master selects the QSPI3 Slave     */
        ...
        IfxPort_PadDriver_cmosAutomotiveSpeed4                          /* Pad driver mode                          */
    };
...
void SPI_SendBuffer(unsigned short int buff, unsigned char buff_size)
{
    unsigned char buff_local[2];

    buff_local[1] = buff >> 8;
    buff_local[0] = buff & 0x00FF;

    //IfxQspi_SpiMaster_exchange(&g_qspi.spiMasterChannel, buff_local, NULL_PTR, 2);

    IfxQspi_SpiMaster_exchange(&g_qspi.spiMasterChannel, &buff_local[1], NULL_PTR, 1);
    
    //while(IfxQspi_SpiMaster_getStatus(&g_qspi.spiMasterChannel) == IfxQspi_Status_busy)
    //{   
    //    /* Wait until the previous communication has finished, if any */
    //}
    ...
    IfxQspi_SpiMaster_exchange(&g_qspi.spiMasterChannel, &buff_local[0], NULL_PTR, 1);
